...
export class AbpValidationSummaryComponent implements OnInit {
  defaultValidationErrors: Partial<AbpValidationError>[] = [
    ...
    { name: 'email', localizationKey: 'InvalidEmailAddress' },
    { name: 'pattern', localizationKey: 'InvalidPattern', propertyKey: 'requiredPattern' },
    { name: 'validateEqual', localizationKey: 'PairsDoNotMatch' },
  ];
  ...
  getValidationErrorMessage(error: AbpValidationError): string {
    ...
    return propertyValue
      ? this.controlEl.nativeElement.getAttribute('localization-key').replace('{0}', propertyValue)
      : this.controlEl.nativeElement.getAttribute('localization-key');
  }
}